/* Génère MOTEUR.md : quatre programmes d'exemple, lisibles comme un pratiquant les lirait.
...
const CAS = [
  ...
  { titre: "Coureur, 4 séances, pour progresser en course à pied", entrees: { frequence: 4, objectif: "mieux", sport: "course", intention: "sport", materiel: "salle", niveau: 2, tempsMin: 60 } }
];
...
let md = `# Moteur de génération — 4 programmes d'exemple
...
Lecture : la semaine indique où tombent les séances (règle 4 : jamais le même gros groupe deux jours de suite, dimanche et lundi compris). Chaque séance liste ses exercices dans l'ordre d'exécution (règle 2), avec séries × reps et repos (règle 5) ; la durée est calculée depuis ces chiffres, échauffement de 6 min compris (règle 6). Le tableau de volume compte les séries par gros groupe et par semaine (règle 3).
...
for (const cas of CAS) {
  ...
  md += `\n## ${cas.titre}\n\n`;
  md += `**Squelette** : ${p.squelette.description} (\`${p.squelette.nom}\`). Niveau observé ${p.entrees.niveau}, ${p.entrees.tempsMin} min par séance, matériel : ${p.entrees.materiel === "salle" ? "salle complète" : p.entrees.materiel}.${p.entrees.sport ? ` Sport : ${M.SPORTS[p.entrees.sport].nom} (intention : progresser dans ce sport).` : ""}\n\n`;
  md += `**Semaine** : ` + p.semaine.map(j => `${JOURS[j.jour - 1]} ${j.seance ? `**${j.seance.lettre}** ${j.seance.nom.toLowerCase()}` : "repos"}`).join(" · ") + `\n\n`;
  for (const s of p.seances) {
    md += `### Séance ${s.lettre} — ${s.nom} · ${s.dureeMin} min\n\n| # | Exercice | Compartiment | Dose | Consigne |\n|---|---|---|---|---|\n`;
    s.exercices.forEach((x, i) => { md += `| ${i + 1} | **${x.nom}** | ${COMP[x.compartiment]}${x.compartiment === "isolation" ? ` (${x.muscle})` : ""} | ${dose(x)} | ${x.consigne} |\n`; });
    md += `\n`;
  }
  md += `**Volume par semaine** (séries, secondaires comptés ½) : ` + M.GROS_GROUPES.map(g => `${g} ${p.volume[g]}`).join(" · ") + ` — bornes du niveau ${p.entrees.niveau} : ${M.NIVEAU[p.entrees.niveau].volumeMin}-${M.NIVEAU[p.entrees.niveau].volumeMax}.\n\n`;
