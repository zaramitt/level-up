...
- 2026-09 — Jambes A / B (v20.14) : dès que la semaine a deux séances jambes (4, 5, 6, 7×), A = dominante quadriceps (ouvre par un squat — squat, presse, extension, fentes — une seule charnière, orientée fessiers), B = dominante chaîne postérieure (ouvre par une charnière — roumain, soulevé — aucun squat, leg curl et hip thrust en isolation ; en force, le soulevé de terre va en B) ; vérifié par la règle `jambesAB` du vérificateur — deux jours jambes identiques ne servaient à rien.
- 2026-09 — Banque (v20.14) : chaque exercice porte des `alias` (autres appellations, français et anglais) et un marqueur `classique` (courant en salle, connu de tous) ; le moteur préfère un classique (+8, puis départage) et ne sort un moins courant que pour varier ou à la demande ; la recherche du panneau matche nom et alias, insensible aux accents, à la casse et aux tirets, une faute tolérée dès 4 lettres ; le filtre muscle montre aussi les muscles secondaires — Léo trouvait ses séances jambes pleines d'exercices qu'il ne fait jamais et ne retrouvait pas le leg extension.
- 2026-09 — Remplacement, dose du remplaçant (v20.14) : séries, reps et repos de l'original sont gardés (la progression continue), sauf isométrie ou fourchette courte (≤ 6 reps) où la dose de la banque s'impose ; l'écran le dit sur chaque candidat (« Même séries et repos que l'exercice remplacé » / « Dose de la banque : … ») — une règle silencieuse passe pour une erreur.
- 2026-09 — Difficulté toujours libellée (« Niveau 3 — avancé »), jamais un chiffre nu ; « Retirer » en rouge discret ; chrono de repos en pilule compacte, agrandie au tap, déplaçable partout, position mémorisée — il chevauchait la carte focus.
...
- 2026-09 — Sauvegarde du KV : cron quotidien (03:00 UTC) vers R2 `level-up-sauvegardes`, un fichier daté, conservation 30 jours ; repli sur un namespace KV daté si R2 absent — une sauvegarde se teste : `outils/sauvegarde.e2e.js` (workerd) fait cycle complet, effacement, restauration octet pour octet.
- 2026-09 — Journal des actions critiques dans le KV (`journal:AAAA-MM`), horodaté, code duo tronqué, aucune donnée libre ; lecture via `GET /admin/journal` protégée par l'en-tête `x-admin-token` (secret `ADMIN_TOKEN`, 404 si absent) — traçabilité sans fuite.
- 2026-09 — CORS : une seule origine autorisée (celle de l'app, `url.origin`), aucun joker ; les autres origines reçoivent 403 — l'API n'est pas faite pour des tiers.
- 2026-09 — Rotation des secrets tous les 90 jours (Anthropic, VAPID, ADMIN_TOKEN), calendrier dans `SECURITE.md` ; `npm audit` à chaque passage sécurité, versions figées dans `package.json`.
- 2026-09 — Environnement de test : second Worker `level-up-test` (env `test` de `wrangler.jsonc`, KV et R2 propres, mêmes crons, variable `ENVIRONNEMENT`), bandeau « VERSION DE TEST » injecté par le worker et pushs préfixés `[TEST]` — même code, deux URLs impossibles à confondre ; la production garde ses liaisons (`keep_vars` pour ne pas perdre les variables du dashboard au déploiement).
- 2026-09 — Flux git : `staging` (→ `level-up-test`) reçoit toutes les PR, Léo valide sur l'URL de test, puis PR `staging` → `main` ; correctif urgent direct sur `main` puis `main` → `staging` — la validation se fait sur une vraie URL, pas sur une capture.
- 2026-09 — Données de test = sauvegarde de production amorcée par `outils/amorcer-test.js`, sans abonnements push, planifications, quotas ni photos, prénoms anonymisés ; c'est aussi la restauration d'essai du backlog — une sauvegarde se prouve en la restaurant pour de vrai.
- 2026-09 — Tentative d'intrusion (tableau dans `SECURITE.md`) : le worker ne doit jamais lever d'exception sur une entrée (corps non objet → 400, URL malformée → 400) ; débit limité aussi en lecture (600/min) et sur `/testpush` (3/min) ; 400 photos par code ; quota IA par adresse (25/jour, empreinte hachée salée par la date, jamais l'adresse) — ce qui exige de distinguer coach et coachée reste au backlog (3 à 4 jours estimés).
